...
const Welcome = () => {
  ...
  useEffect(() => {
    const fetchSegments = async () => {
      try {
        const response = await fetch('https://localhost:7185/api/segments/');
  ...
  useEffect(() => {
    const fetchManufacturers = async () => {
      ...
      try {
        const response = await fetch(`https://localhost:7185/api/manufacturers/${segment}`);
  ...
  useEffect(() => {
    const fetchVariants = async () => {
      ...
      try {
        const response = await fetch(`https://localhost:7185/api/models/${segment}/${manufacturer}`);
  ...
  const handleSubmit = (e) => {
    ...
    navigate('/vehicle-details', {
      state: { segment, manufacturer, variant, quantity },
    });
  };
      ...
      <form onSubmit={handleSubmit} className="welcome-form">
        ...
        <div className="form-group">
          ...
          >
            ...
            {segments.length === 0 ? <option disabled>Loading...</option> :
              segments.map((seg) => (
        ...
        <div className="form-group">
          ...
          >
            ...
            {manufacturers.length === 0 ? <option disabled>Loading...</option> :
            ...
            {loadingVariants ? (
              ...
              variants.map((varnt) => (
                <option key={varnt.id} value={varnt.id}>
                  {varnt.modName} {/* Use modName here */}
                </option>
              ))
            )}
          </select>
