...
const VehicleDetails = () => {
  ...
  useEffect(() => {
    const fetchVehicleDetails = async () => {
      try {
        const vehicleResponse = await fetch(`https://localhost:7185/api/models/${variant}`);
        if (!vehicleResponse.ok) throw new Error('Failed to fetch vehicle details');
        const vehicleData = await vehicleResponse.json();
        setVehicleDetails(vehicleData);

        const componentsResponse = await fetch(`https://localhost:7185/api/vehicle-components/${variant}`);
